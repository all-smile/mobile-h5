<template>
  <div class="container">
    <button class="btn" @click="stopVideoDevice">关闭摄像设备</button>
    <button class="btn" @click="startVideoDevice">开启摄像设备</button>
...
export default {
  ...
  methods: {
    stopVideoDevice() {
      console.log('stop bdFaceLiveDetection', this.bdFaceLiveDetection);
      // this.bdFaceLiveDetection.close() // 关闭摄像头
      this.bdFaceLiveDetection.destroy() // 关闭摄像头
    },
    startVideoDevice() {
      this.reStart()
      // this.bdFaceLiveDetection.restart() // 开启摄像头
    },
    async init(query) {
      // 区分是否使用安全
